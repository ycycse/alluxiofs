...
class SegmentRanges:
    ...
    def get(self, index):
        if index < 0 or index >= self.segment_num:
            raise Exception(f"index {index} is out of range in segmentRanges of {self.segment_num}")
        if index == 0:
            return SegmentRange(self.file_path, self.first_segment_index, self._first_segment_offset(), self._first_segment_length())
